...
(() => {
  ...
  const io = new IntersectionObserver(
    (entries) =>
      entries.forEach(
        (en) => en.isIntersecting && en.target.classList.add("in-view")
      ),
...
(() => {
  ...
  const layout = () => {
    ...
    requestAnimationFrame(() => {
      ...
      viewport.scrollTo({
        left: Math.min(snapLeft, maxScrollLeft()),
        behavior: "auto",
      });
...
(() => {
  ...
  thumbs.forEach(async (a) => {
    ...
    try {
      await preload(maxres);
      a.style.backgroundImage = `url("${maxres}")`;
    } catch {
      a.style.backgroundImage = `url("${hq}")`;
    }
